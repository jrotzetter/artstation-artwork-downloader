...
class ArtStationArtworkDownloader(tk.Tk):
    def __init__(self):
        ...
        self.storepath_lbl = ttk.Label(
            master=self.options_frm, text=" Downloads will be saved to:"
        )
        self.storepath_ent = ttk.Entry(
            master=self.options_frm,
        ...
        self.storepath_lbl.grid(row=0, column=2, padx=5, pady=(5, 0), sticky="W")
        self.storepath_ent.grid(row=1, column=2, padx=10, pady=(0, 10), sticky="EW")

        ###/// GET JSON FRAME \\\###
        self.get_json_frm = ttk.Frame(master=self.main_frm, relief="groove")

        self.get_json_lbl = ttk.Label(master=self.get_json_frm, text="Paste hash ID:")
        self.get_json_ent = ttk.Entry(
        ...
        self.get_json_frm.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        self.get_json_frm.grid_rowconfigure(
            (0, 1), weight=1
        )  # center widgets vertically by giving them equal weight
        self.get_json_frm.grid_columnconfigure(
            (0, 1), weight=1
        )  # center widgets horizontally by giving them equal weight

        self.get_json_lbl.grid(row=0, column=0, padx=5, pady=(5, 0))
        self.get_json_ent.grid(row=1, column=0, padx=10, pady=(0, 10))
        self.get_json_btn.grid(row=1, column=1, padx=10, pady=(0, 10))

        ###/// LOAD JSON FRAME \\\###
        self.load_json_frm = ttk.Frame(master=self.main_frm, relief="groove")

        self.load_json_btn = ttk.Button(
            master=self.load_json_frm,
            text="Load JSON from clipboard",
            width=25,
            command=self._get_clipboard_content,
        )
        ...
        self.load_json_frm.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        self.loaded_json_lbl.grid(row=0, column=1, padx=5, pady=(5, 0))
        self.loaded_json_ent.grid(row=1, column=1, padx=10, pady=(0, 10))
        self.clear_json_btn.grid(row=1, column=2, padx=10, pady=(0, 10))
        self.load_json_btn.grid(row=1, column=0, padx=10, pady=(0, 10))
        self.load_json_frm.grid_rowconfigure((0, 1), weight=1)
        self.load_json_frm.grid_columnconfigure((0, 1, 2), weight=1)
        ...
        img_y_scrollbar.config(command=self.images_list.yview)
        img_x_scrollbar.config(command=self.images_list.xview)

        self.output_frm.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        self.output_frm.grid_rowconfigure(0, weight=1)
        self.output_frm.grid_columnconfigure(0, weight=1)

        self.images_list.grid(row=0, column=0, padx=5, pady=5, sticky="EW")
        ...
        self.custom_entry = ttk.Entry(master=self.run_frm, width=25, state="disabled")
        ...
        self.run_frm.grid_columnconfigure((0, 1, 2), weight=1)

        self.custom_entry.grid(row=0, column=1, padx=10, pady=10)
        self.custom_name_check.grid(row=0, column=0, padx=10, pady=10)
    ...
    def _show_use():
        messagebox.showinfo(
            ...
            "3. Visit and copy JSON content from resulting URL\n"
    ...
    def _download_images(self):
        ...
        def download_image(url: str, filename: str, storepath: str) -> str:
            def get_extension(url: str, resp: requests.Response) -> str:
                # Get extension from Content-Type header...
                content_type = (
                    resp.headers.get("Content-Type", "").split(";")[0].strip().lower()
                )
                extn = EXTN_FROM_CONTENT_TYPE.get(content_type)
            ...
            try:
                ...
                with session.get(
                    ...
                    ext = get_extension(url_no_cache, resp)
                    file = f"{filename}{ext}"
                    filepath = os.path.join(storepath, file)
        ...
        selections = self.images_list.curselection()

        all_items = self.images_list.get(0, tk.END)
        selected_images = [
        ...
        with requests.Session() as session:
            if custom_name_check and not custom_name == "":
                ...
                for image in selected_images:
                    ...
                    download_result = download_image(image_url, filename, store_path)
        ...
        self.log_lb.insert(
            tk.END,
            f">>> {progbar_max} Files - Skipped: {self.SKIPS}, Errors: {self.ERRORS}",
        )
